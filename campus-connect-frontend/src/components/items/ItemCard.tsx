...
export function ItemCard({ item }: ItemCardProps) {
  ...
  // This 'ownerId' check is critical and relies on your backend sending it.
  ...
  return (
    <div className="border rounded-lg shadow-lg bg-white relative overflow-hidden transition-transform hover:shadow-xl hover:scale-[1.01] flex flex-col">
      ...
      <div className="p-4 flex flex-col flex-grow">
        <span className={`inline-block px-3 py-1 text-xs font-semibold rounded-full mb-2 self-start ${
          item.type === "FOUND"
        ...
        <h3 className="text-xl font-bold mb-2 truncate">
          <Link href={`/items/${item.id}`} className="hover:text-blue-600">{item.title}</Link>
        </h3>
