...
function MyClaimsContent() {
  ...
  useEffect(() => {
    const fetchClaims = async () => {
      try {
        ...
        setClaims(data.reverse()); // Show newest first
      } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading && <div className="text-center py-10">Loading my claims...</div>}
      ...
      {!loading && !error && claims.length === 0 && (
        <div className="text-center text-gray-500 py-10 border-2 border-dashed rounded-lg bg-gray-50">
          ...
          <Link href="/items" className="mt-4 inline-block bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
            Browse Items
          </Link>
        </div>
      )}
              ...
              {claims.map((claim) => (
                <tr key={claim.id}>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <Link href={`/items/${claim.itemId}`} className="text-blue-600 hover:underline">
                      View Item
                    </Link>
                  </td>
